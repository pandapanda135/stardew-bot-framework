...
{
    public static ShopMenu? CurrentShop;
    ...
    public int? TabAmount
    {
        get
        {
            ...
            {
                return CurrentShop.tabButtons.Count;
            }
    ...
    {
        if (CurrentShop is null || CurrentShop is not ShopMenu) return;
        
        if (index < 4)
        {
            for (int i = 0; i < quantity; i++)
            { CurrentShop.receiveLeftClick(CurrentShop.forSaleButtons[index].bounds.X + 10, CurrentShop.forSaleButtons[index].bounds.Y + 10, true); }
            return;
        }
        ...
        {
            CurrentShop.receiveLeftClick(CurrentShop.downArrow.bounds.X, CurrentShop.downArrow.bounds.Y, true);   
        }
        
        // buy amount
        int maxIndex = CurrentShop.forSaleButtons.Count - 1;
        for (int i = 0; i < quantity; i++)
        {
            Logger.Info($"buying click {i}");
            CurrentShop.receiveLeftClick(CurrentShop.forSaleButtons[maxIndex].bounds.X, CurrentShop.forSaleButtons[maxIndex].bounds.Y, true);    
        }
        ...
        for (int i = index; i >= 4; i--)
        {
            ...
            CurrentShop.receiveLeftClick(CurrentShop.upArrow.bounds.X, CurrentShop.upArrow.bounds.Y, true);   
        }
    }
    ...
    public void BuyItem(Item item, int quantity)
    {
        ...
        for (int i = 0; i < CurrentShop.forSale.Count; i++)
        {
            if (CurrentShop.forSale[i].Name == item.Name)
            {
                BuyItem(i, quantity);
                return;
            }
    ...
    {
        if (CurrentShop is null || CurrentShop is not ShopMenu) return null;
        
        return CurrentShop.forSale;
    }
    ...
    {
        if (CurrentShop is null || CurrentShop is not ShopMenu)
        {
            currency = -1;
        ...
        items = CurrentShop.forSale;
        return CurrentShop.itemPriceAndStock;
    }
    ...
    public void ChangeTab(int newTab)
    {
        ...
        if (CurrentShop.tabButtons.Count == 0) return;
        
        CurrentShop.switchTab(newTab);
    }
    ...
    public int SellBackItem(int index) // Item item
    {
        ...
        if (CurrentShop is null) return -1;
    
        if (!CurrentShop.CanBuyback())
        {
            Logger.Error($"shop cannot buy back");
            return -1;
        }
    
        List<ClickableComponent> inventory = CurrentShop.inventory.inventory;
    
        int itemSalePrice = CurrentShop.inventory.actualInventory[index].GetSalableInstance().sellToStorePrice();
        int itemStackSize = CurrentShop.inventory.actualInventory[index].Stack;
        
        CurrentShop.receiveLeftClick(inventory[index].bounds.X + 3,inventory[index].bounds.Y + 3);
